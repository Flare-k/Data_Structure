...
public:
    ...
    /*
     Master List에 삭제가 이루어질 때, leaf 노드의 값을 삭제의 위치에 옮기고 옮겨진 값 보다 자식 노드의 값이 클 경우 반복적으로 자리를 바꿔준다.
     즉, 좌우 큰 노드를 찾고 둘 중 큰 노드와 현재 노드의 대소를 비교한다. 현재 노드보다 크다면 서로 자리를 바꿔준다. 위에서 아래 방향으로 진행한다.
     */
    virtual void ReheapUp(int iroot, int ibottom);
    /*
     Master List에 추가가 이루어질 때, leaf node가 root node보다 큰 경우 두 노드의 값을 Swap해준다. (밑에서 위로 올라가며 swap)
     */
...
template<typename T>
void MaxHeap<T>:: ReheapDown(int iparent, int ibottom){ //삭제할 때 쓰임.
    int maxChild;
    ...
    if(leftChild <= ibottom){//길이가 6이면 이때 ibottom은 5
        if(leftChild == ibottom)
            maxChild = leftChild;       //left랑 ibottom이 같으면 당연히 left가 max
        else{
        ...
        if(HeapBase<T>::m_pHeap[iparent] < HeapBase<T>::m_pHeap[maxChild]){
            HeapBase<T>::Swap(iparent, maxChild);
...
template<typename T>
void MaxHeap<T>:: ReheapUp(int iroot, int ibottom){ //root = 0 , bottom = 현재 길이 값
    int parent_node;
    if(ibottom > iroot){
        parent_node = (ibottom-1)/2;        //(6-1)/2 = 2
        if(HeapBase<T>::m_pHeap[parent_node] < HeapBase<T>::m_pHeap[ibottom]){
            HeapBase<T>::Swap(parent_node, ibottom);  //부모노드보다 자식노드가 크면, Swap(부모노드, 자식노드)   parent = 2, ibottom = 6
...
//Delete(item, found, 0);
template<typename T>
void MaxHeap<T>:: Delete(T item, bool& found, int iparent){     //iparent = 0
    int leftChild = iparent*2 + 1;
    int rightChild = iparent*2 + 2;
...
template<typename T>
void MaxHeap<T>::Replace(T &item, bool &found, int iparent){
    int leftChild = iparent*2 + 1;
    int rightChild = iparent*2 + 2;
